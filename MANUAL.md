...
#### 左侧栏
- **按组织**：点击筛选，点击"排序"可拖拽调整组织顺序。排序模式下可将组织拖过分界线，改变其所属层级（主要/更多）。若出现新组织，会自动追加到列表末尾参与排序。
...
### 数据删除
- **删除所有模型数据**：清除所有模型、排序、分组数据，但保留语言、云同步等个人设置。
- **删除所有数据**：彻底清空所有数据和缓存，页面将自动刷新并恢复全新状态。
...
A: 脚本会优先尝试从 Arena 下拉栏实时提取该模型的原生 SVG 图标；若未扫描到，则尝试加载 GitHub 上的官方 Logo；若依旧失败，会使用 emoji 作为回退。请尝试打开 Arena 下拉框触发自动扫描以获取原生图标。

**Q: 悬浮按钮在某些页面不显示？**
A: 这是正常行为。自v5.0.0，脚本仅在 Arena 的对话页面显示管理按钮，在排行榜、博客等页面会自动隐藏。
