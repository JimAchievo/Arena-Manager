...
#### 🔧 问题修复
- **修复左栏初始不显示"更多组织"**：解决清除数据重新扫描后，部分模式左栏初始状态下 Tier 2 组织无法显示，需点击排序才出现的 Bug。
- **修复排序模式 UI 白底白字**：解决进入排序模式时，原选中组织的背景色恢复但字体颜色未恢复导致不可读的问题。

#### ✨ 优化
- **左栏排序状态优化**：进入排序模式时，被选中的组织与其他组织平权显示，隐藏选中态但保留记忆，退出排序后恢复选中外观。
- **选中图标底色优化**：左栏组织被选中（蓝底）时，为图标增加半透明白色底衬，解决蓝色系图标（如 Meta、DeepSeek）在蓝底上不可见的问题。
- **外置配置文件重命名**：`org-config.json` 拆分重组为 `company-rules.json`，仅保留组织前后缀识别规则，组织层级与顺序回归内置配置及用户自定义。
...
#### 🐛 问题修复
- **修复分组计数不准确**：删除模型后，分组在顶栏显示的计数可能大于实际模型数
- **修复 `setVisibility` 内部调用参数错误**：极少数边缘情况下新建模型时模式归属可能异常
- **修复多选模式还原不完整**：还原操作现在能正确撤销所有可见性变更（包括切换标签页后的操作）和分组变更（添加至分组/移出分组），恢复到进入多选模式前的确切状态

#### 🔧 内部优化
- 删除模型时自动清理分组引用和自定义排序残留，保持数据整洁
- 组织排序数据中不再残留 `---` 分隔符
- `analyze` 方法参数 `strictMode` 重命名为 `mode`，更准确表达含义

### v5.0.0 (2026-03-15)

#### 🎉 重要变更
- **🧹 数据管理重构**：「重置所有数据」拆分为两个选项
  - **删除所有模型数据**：仅清除模型、排序、分组，保留个人设置
  - **删除所有数据**：彻底清空所有数据和缓存，恢复全新状态
- **📍 智能页面检测**：在非对话页面（如排行榜、博客等）自动隐藏悬浮按钮和快捷键
- **🎬 Video 标签自动隐藏**：当没有 Video 模型时，上栏不再显示 Video 选项

#### 🔧 改进
- 移除「应用」按钮，关闭面板时自动同步到原生下拉框
- 移除基于图标名称的组织二次识别逻辑（`ICON_TO_ORG`），简化代码

### v4.7.4 (2026-02-22)

#### 🎉 新增功能
- **🏢 组织官方 Logo**：模型卡片和侧边栏中的 emoji 图标替换为组织官方 Logo（SVG/PNG），首次加载后本地缓存，加载失败自动回退为 emoji
- **📂 跨 Tier 拖拽排序**：排序模式下，组织可以跨越 Tier 1 / Tier 2 分界线自由拖拽，重新分配所属层级

#### 🔧 改进
- 模型详情面板中，内置组织的图标改为只读显示官方 Logo；手动匹配的组织（Other）仍可编辑自定义图标

### v4.7.2 (2026-02-15)

#### 🎉 新增功能
- **⭐ 推荐模型配置**：设置面板新增「推荐模型配置」区域
  - 一键从 GitHub 仓库下载作者推荐的模型设置
  - **差异对比面板**：下载后自动与本地数据比较，分类展示差异
  - **选择性合并**：可见性、排序、收藏、模型信息（备注/图标/组织）、分组五大类别独立勾选
  - 不会覆盖用户个人设置（语言、Token 等）
  - 本地不存在的模型会自动添加，本地多出的模型不会删除
  - 显示本地已导入版本日期和最新可用版本日期（精确到分钟）

#### 🔧 修复
- 修复 GitHub raw 文件 CDN 缓存导致刚上传的配置无法立即读取的问题
